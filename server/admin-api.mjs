...
import {sendConfirmation} from './gmail.mjs';
...
const safeDoc=doc=>{const data=unpack(doc);if(!data)return null;const result={id:doc.name.split('/').pop(),version:doc.updateTime};for(const key of ['reference','name','email','phone','service','message','context','kind','quantity','addWorkshop','totalOMR','status','notes','createdAt','updatedAt','emailStatus','emailAttempts','emailSentAt','emailAttemptAt'])if(key in data)result[key]=data[key];return result;};
export async function adminAPI(request,env,{auth=authenticate,db=database(env.FIREBASE_SERVICE_ACCOUNT_JSON),notify=sendConfirmation}={}){
 ...
 try{
  ...
  const match=route.match(/^requests\/([^/]+)(\/email)?$/);if(!match||!validId(match[1]))return reply(404,{error:'not_found'});
  ...
  if(request.method==='POST'&&match[2]){await notify(id,env);return reply(200,{request:safeDoc(await db.read(path))});}
  ...
  if(!['new','contacting','in_progress','completed','archived'].includes(input.status)||typeof input.notes!=='string'||input.notes.length>4000||input.version!==doc.updateTime)return reply(409,{error:'invalid_or_stale'});
  await db.patch(path,{status:input.status,notes:input.notes.trim(),updatedAt:new Date().toISOString()},doc.updateTime);
