...
import {ledgerAPI} from './ledger.mjs';
import {consultationAdmin} from './consultations.mjs';
...
export async function adminAPI(request,env,{auth=authenticate,db=database(env.FIREBASE_SERVICE_ACCOUNT_JSON),notify=sendConfirmation,notifyOwner=sendOwnerNotification}={}){
 ...
 try{
  if(route==='consultations'||route.startsWith('consultations/'))return await consultationAdmin(request,env,db,route);
  if(route==='ledger'||route.startsWith('expenses/'))return await ledgerAPI(request,env,db,route);
