...
const servicesData = [
 {visual:'training',image:'assets/images/be5a3c6d74a6e1fb.jpg',imageAr:'تدريب عملي ومتابعة أثناء التطبيق',imageEn:'Hands-on training during practical work',ar:'الورش والتدريب التقني',en:'Technical Workshops & Training',descAr:'تجارب عملية مصممة حسب الفئة والهدف، من الفكرة إلى التطبيق.',descEn:'Hands-on experiences shaped around the audience and goal, from concept to practice.',
...
function closeModal(){const box=document.getElementById('modalOverlay');if(!box.classList.contains('open'))return;box.querySelectorAll('video').forEach(video=>{video.pause();video.removeAttribute('src');video.querySelectorAll('source').forEach(source=>source.removeAttribute('src'));video.load();});box.classList.remove('open');box.inert=true;document.body.style.overflow=projectBodyOverflow||'';projectTrigger?.focus({preventScroll:true});}
