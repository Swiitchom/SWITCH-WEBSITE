...
- public/assets/projects.js: 8 مشاريع ومنصات؛ تتضمن المشاريع الستة السابقة والمنصتين المستخرجتين من اختصارات سطح المكتب والصور الجديدة. أزيلت الوحدتان الإلكترونيتان بناءً على طلب صاحب الموقع. أضيفت لقطات المنصتين وصورة الخوذة، وبقي القفاز بأيقونته دون صورة.
...
### Guided quote details and selection
...
### Private invoices, delivery and monthly ledger

`/admin/invoices.html` saves editable invoices in `portfolioInvoices`. Preparing a PDF renders the saved snapshot locally in the browser with Arabic shaping and pagination, without an external conversion service or public file URL. Downloading and native file sharing are explicit owner actions. If native sharing is unavailable, the owner downloads the PDF and attaches it in WhatsApp manually. A share result is not a delivery receipt.

Email delivery requires a saved customer email, a prepared PDF and an explicit recipient confirmation. The owner-only, same-origin `POST /api/admin/invoices/:id/email` checks the invoice version, then sends a MIME PDF attachment through the existing Gmail account. An atomic lease and the shared daily mail quota bound delivery. Identical successful deliveries are deduplicated; uncertain outcomes require checking Gmail Sent and are not automatically retried. Invoice data, attachment contents and credentials are not logged or submitted to analytics. PDFs are not published in the repository or static website.

`/admin/ledger.html` reports all matching monthly records, including paginated results beyond the invoice list. A paid invoice contributes its full total on its explicit `paidDate`; drafts, issued-but-unpaid and cancelled invoices are not receipts. Expenses use their paid date and integer baisa amount. Voiding an expense retains the record and excludes it from totals. Net cash movement is recorded receipts minus active expenses, not an accrual profit report or partial-payment ledger. Historical paid invoices need a received date before their income can be included.

All financial APIs use the existing owner session; Firestore client access is denied. Records remain in Firebase and are used for this site's owner workflows. Only the PDF explicitly selected by the owner is shared with the recipient; tests use synthetic data and mocked mail, never customer messages.
...
`/consultation.html` lists owner-configured types, OMR prices and available Muscat times. Manage availability with a monthly calendar at `/admin/consultations.html` (also linked from invoices). The consultation name and price are already configured; owners click days and time buttons without entering names. Visitors select an available day, then a time, then provide contact details. Bookings create private inquiries and use the existing email outbox. No payment is taken online. Slot reservation, price version, rate limit and request reference are committed atomically. Overlapping availability is rejected; booked slots cannot be deleted. Public responses exclude customer data. No default prices are deployed by the source code.
